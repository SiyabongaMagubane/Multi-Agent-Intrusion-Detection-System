...
public class KNearest implements classificationInterface {
	...
	private void ReadInput() {
		...
		radioPanel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), "Ratio"));
		...
		for (int count = 0; count < getSize(dataset); count++) {
			...
			for (int i = 0; i < dataset.get(count).length; i++) {
				...
				case SERROR_RATE:
					...
					this.Extracted_Dataset[count][newCount] = dataset.get(count)[i];
					// this.Extracted_Dataset[count][newCount] = (int)
					// (Double.parseDouble(this.TrainingDataset.get(count)[i]));
	...
	public void FeatureExtractionTraining() {
		...
		// System.out.println(" Performing Feature Extraction on training
		// dataset");
	...
	/*
	 * Load the training and test data choose the value of k for each point in
	 * test data: - find the Eucledian distance to all training data points -
	 * store the Eucledian distances in a list and sort it choose the first k
	 * points - assign a class to the test point based on the majority of the
	 * classes present in the chosen points - end
	 *
	 * 
	 */
	public void KNN() {
		...
		// Testing sorting
		/*
		 * for(int i=0; i < this.Extracted_TestDataset.length; i++) { for(int
		 * k=0; k < this.Extracted_TestDataset[i].length; k++) { double min =
		 * 0.0; // Set To Your Desired Min Value double max = 10.0; // Set To
		 * Your Desired Max Value double ran = (Math.random() * ((max - min) +
		 * 1)) + min; // This Will Create
		 * 
		 * double xrounded = Math.round(ran * 100.0) / 100.0;
		 * System.out.print(xrounded); //System.out.print(" , ");
		...
		for (int p = 1; p < (this.Extracted_TestDataset.length - 1); p++) {
			...
			for (int i = 1; i < this.Extracted_TrainingDataset.length; i++) {
				...
				// System.out.printf(" distance value of row %i in col %i : %s
				// %n", i, k ,EucledianDistanceUnsorted.get(i).get(k));
			}

			// start at index 1 discard the attribute name row
			/*
			 * for ( int i = 0; i < (this.Extracted_TrainingDataset.length-1) ;
			 * i++) { System.out.print(" distance value of row ");
			 * System.out.print(p); System.out.print(" in col ");
		...
		// find majority distance and find index of distance in unsorted
		// distance
		double majority = 0.0;
		int IndexofDistance = 0;
		for (int i = 1; i < (EucledianDistanceSorted.size() - 1); i++) {
			// trying to find of random selected EucledianDistances
			...
			confusionmatrix.addPredictionNumber(majority);
			confusionmatrix.addTrueValueNumber(EucledianDistanceUnsorted.get(i).get(i));
			...
			confusionmatrix
					.addPrediction(Extracted_TrainingDataset[IndexofDistance][this.feature_extraction.size() - 1]);

		}

		/*
		 * Curve analysis = new Curve.PrimitivesBuilder()
		 * .predicteds(confusionmatrix.getPredictions())
		 * .actuals(confusionmatrix.getTrueValues()) .build();
		 * 
		 * // Calculate AUC ROC double area = analysis.rocArea();
		 * 
		 * // Convex hull Curve hull = analysis.convexHull(); double maxArea =
		 * hull.rocArea();
		 * 
		 * // Points for plotting the curve double[][] points =
		 * hull.rocPoints();
		 */
		
		
	}
	public void VisualizeClassification(){
		...
		//ReceiverOperatingCharacteristics roc = new ReceiverOperatingCharacteristics(expected,
			//	score);
		ROCS.add(roc);
		...
		space.setTitle("ROC curve diagram with k value of :" + this.ValueofK);
	...
	public int[] ClassificationResultTrueValues() {
		return confusionmatrix.getTrueValues();
	}

	public double[] ClassificationResultPredictions() {
		return confusionmatrix.getPredictions();
	}
	
	public int TotalNumberOfPredictions(){
		return confusionmatrix.TotalNumberOfPredictions();
	}
	public int Incorrect(){
		return confusionmatrix.Incorrect();
	}
	public int Correct(){
		return confusionmatrix.Correct();
	}
	public int TruePositive(){
		return confusionmatrix.TruePositive();
	}
	public int FalsePositive(){
		return confusionmatrix.FalsePositive();
	}
	public int TrueNegative(){
		return confusionmatrix.TrueNegative();
	}
	public int FalseNegative(){
		return confusionmatrix.FalseNegative();
	}
	public double ErrorRate(){
		return confusionmatrix.ErrorRate();
	}
	public double AccuracyRate(){
		return confusionmatrix.AccuracyRate();
	}
	public double TruePositiveRate(){
		return confusionmatrix.TruePositiveRate();
	}
	public double FalsePostiveRate(){
		return confusionmatrix.FalsePostiveRate();
	}
	
	
	public ArrayList<String> trueValues(){
		return confusionmatrix.TrueValues() ;
	}
	
	public ArrayList<String> predictionValues(){
		return  confusionmatrix.PredictionValues();
	}
	...
	{  
		...
		// apply the randomization of selecting the first k values and find a majority between them.
		int max = arrayList.size();
		ArrayList<Double> randomizedArrayList = new ArrayList<Double>();
		for ( int k = 0; k < n;k++){
			randomizedArrayList.add(arrayList.get(getRandomBetweenRange(0,max-1)));
		}
	    ...
	    {  
	        ...
	        for(int j = 0; j < n; j++)  
	        {  
	            if(randomizedArrayList.get(i).equals(randomizedArrayList.get(j))) {       
	            count++; 
	            }
